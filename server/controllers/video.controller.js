...
export const getVideo = AsyncTryCatch(async (req, res, next) => {
  ...
  if (!token) return res.status(200).json({ video, isLiked: false, loggedIn: false });
...
export const getComments = AsyncTryCatch(async (req, res, next) => {
  ...
  if (video.canComment === false) return next(new ErrorHandler(400, "Comments are disabled for this video."));
