...
import { GET_COMMENTS, GET_PLAY_NEXT, GET_VIDEO, LIKE_UNLIKE, PUT_COMMENT } from '../../utils/constants';
...
const VideoPlayer = () => {
    ...
    const putComment = async () => {
        ...
        try {
            const response = await axios.post(`${PUT_COMMENT}/${videoId}`, { text: commentText }, { withCredentials: true });
    ...
    useEffect(() => {
        const getVideoData = async () => {
            try {
                const response = await axios.get(`${GET_VIDEO}/${videoId}`, { withCredentials: true });
    ...
    const fetchComments = async () => {
        const response = await axios.get(`${GET_COMMENTS}/${videoId}?limit=2&skip=0`);
    ...
    const loadMoreComments = async () => {
        ...
        console.log('init 2')
        const response = await axios.get(`${GET_COMMENTS}/${videoId}?limit=2&skip=${newSkip}`);
        console.log(response)
        console.log('init 3')
        if (response.data.comments?.length > 0) {
            console.log('yes len is more')
            setComments((prevComments) => { console.log("in cnt", response.data.comments.length); return [...prevComments, ...response.data.comments] });
            setSkip(newSkip);
        }
    ...
    const handleLike = async () => {
        ...
        try {
            console.log(!isLiked)
            const response = await axios.patch(`${LIKE_UNLIKE}/${videoId}`, { isLiked: !isLiked }, { withCredentials: true });
    ...
    return (
        <div style={{
            ...
            <div className="flex flex-col w-full h-auto max-w-full lg:w-[60%] box-border">
                        ...
                        <div className="p-2 h-auto box-border">
                            ...
                            <div className="min-h-[50px] h-auto font-roboto text-xl text-white flex flex-wrap xs:flex-nowrap xs:justify-between box-border">
                                ...
                                <div className="w-full xs:w-auto mt-3 xs:mt-0 xs:justify-end flex items-center justify-evenly box-border">
                                    <div onClick={handleLike} className="w-[80px] h-[36px] rounded-3xl flex flex-row justify-evenly hover:bg-[#635f5f] px-3 text-sm items-center bg-[#2e302f] box-border">
                                        {isLiked ? <><ThumbUpAltIcon /> </> : <><ThumbUpOffAltIcon /></>} {likes}
                                    </div>
                    ...
                    <div className="w-full  h-auto max-w-[100vw] overflow-hidden box-border">
                        {videoDetails && comments ?
                            <div className='flex flex-col w-full h-auto max-w-[100vw] mt-4 overflow-hidden box-border'>
                                <div className='pl-3 font-roboto font-bold text-lg'>{videoDetails.commentCount} Comments</div>
                                <div className='flex flex-row mt-5'>
                                    <div className='rounded-full h-[50px] w-[50px]'>
                                        <img className='object-contain w-full h-full rounded-full' src={videoDetails.channel?.profilePhoto} />
                                    </div>
                                    <div className='flex ml-3 w-full flex-col'>
                                        <div>
                                            <TextField id="standard-basic" value={commentText} onChange={(e) => setCommentText(e.target.value)} variant="standard" multiline className='w-full' />
                                        </div>
                                        <div className='flex flex-row h-[50px] items-center justify-between'>
                                            <MdOutlineEmojiEmotions className='h-[30px] w-[30px]' />
                                            <div className='flex flex-row'>
                                                <div className='rounded-3xl px-3 py-1 hover:bg-slate-600 mr-3 cursor-pointer '>Cancel</div>
                                                <div onClick={putComment} className={`mr-1 rounded-3xl px-3 py-1 bg-blue-500  text-black font-roboto ${commentText.length === 0 ? "disabled bg-slate-400" : "hover:bg-blue-300"} font-semibold cursor-pointer`}>Comment</div>
                                            </div>
                                        </div>
                                        ...
                                        return (
                                            <div className='flex flex-row my-5' key={comment?._id}>
                                                <div className='flex w-[40px] h-[40px] rounded-full mr-2'>
                                                    <img className='w-full h-auto rounded-full' src={comment?.channel.profilePhoto} />
                                                </div>
                                                ...
                                                <div className={`${comment?.channel._id === channelInfo?._id || videoDetails.channel._id === channelInfo?._id ? 'block' : 'hidden'}`}>
                                                    <BsThreeDotsVertical className='mt-2 ml-2' onClick={handleCommentMenuClick} />
                                                    <Menu
                                                        id="basic-menu"
                                                        anchorEl={anchorEl}
                                                        open={open}
                                                    ...
                                                    >
                                                        <MenuItem className={`${comment?.channel._id === channelInfo?._id ? 'block' : 'hidden'}`} onClick={handleCommentMenuClose}>Edit</MenuItem>
                                                        <MenuItem onClick={handleCommentMenuClose}>Delete</MenuItem>
                                                    </Menu>
                                                </div>
                                            </div>
                ...
                ) : <div className='flex mx-auto mt-4'>Comments are disabled for this video</div>}
