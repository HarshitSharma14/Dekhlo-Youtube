...
const SearchPage = () => {
    ...
    const getSearchResults = async () => {
        ...
        try {
            ...
            const { data } = await axios.get(SEARCH_VIDEO_ROUTE, { params: { searchText, cursor } })
            console.log(data)
    ...
    return (
        <div className='max-w-[1200px] mx-auto mt-5'>
            {videos.map((video, index) => (
                <div key={video._id} onClick={() => navigateToVideo(video._id)} ref={index === videos.length - 1 ? lastElementRef : null}>
                    <LongVideoCard video={video} />
                </div>
            ))}
            {loading && <div className='flex justify-center items-center mt-5'>Loading...</div>}
            {!hasMore && <div className='flex justify-center items-center mt-5'>No more videos to show.</div>}
        </div>
