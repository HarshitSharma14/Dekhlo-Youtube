...
const Header = ({ isDisabled }) => {
  ...
  // Handle click outside to reset search state
  useEffect(() => {
    const handleClickOutsideSearch = (event) => {
      // Only handle if we're in search mode
      if (!back) return;

      // Check if click is outside search container and not on search button
      const isOutsideSearch = !event.target.closest(".search-container");
      const isNotSearchButton = !event.target.closest(".search-button");

      if (isOutsideSearch && isNotSearchButton) {
        // Prevent the click from reaching other elements
        event.preventDefault();
        event.stopPropagation();
        setBack(false);
      }
  ...
  return (
    <>
      {/* Backdrop overlay when in search mode */}
      {back && (
        <div
      ...
      <header className="sticky top-0 bg-[#121212] shadow-none h-[70px] z-50">
        <div className="flex h-full pr-2 justify-between items-center">
          {/* Left Section: Hamburger Icon & Logo */}
          <div className={`${back ? "hidden" : "flex"}`}>
            <YoutubeIconSection />
          </div>

          {/* Back Button - Only visible in search mode */}
          {back && (
            <button
              onClick={(e) => {
                e.stopPropagation();
              ...
              className="ml-4 p-2 hover:bg-[#303030] rounded-full transition-colors duration-200"
            >
              <svg
          ...
          <div
            className={`search-container ${
              back ? "flex ml-3 mr-4 flex-1" : "hidden"
            } items-center border border-s-2 border-[#303030] h-[40px] rounded-3xl bg-[#121212] overflow-hidden xs:flex ${
              !back ? "w-[600px] max-w-full" : ""
            }`}
          >
            <Autocomplete
              freeSolo
          ...
          {isLoggedIn ? (
            <div className={`${back ? "hidden" : "flex"} items-center`}>
              <button
                disabled={isDisabled}
                className="search-button xs:hidden p-2 mr-2 justify-self-end flex items-center justify-center hover:bg-[#303030] rounded-full transition-colors duration-200"
                onClick={(e) => {
                  e.stopPropagation();
                  setBack(true);
                  // Auto-focus the search input after a short delay to ensure DOM is ready
                  setTimeout(() => {
              ...
              <button
                ...
                disabled={isDisabled}
                className="text-white ml-2  flex flex-row w-auto  bg-[#222222] hover:bg-gray-700 p-2 mr-2 xxs:w-[90px] h-[40px] border border-s-2  rounded-3xl border-[#303030]"
              >
            ...
            <div className={`${back ? "hidden" : "flex"} items-center`}>
              ...
              <button
                ...
                className=" w-32 group ml-2 relative bg-[#303030] hover:bg-[#404040] text-white font-medium py-2.5 px-5  rounded-full transition-all duration-200 border border-[#404040] hover:border-[#505050] shadow-sm hover:shadow-md"
              >
